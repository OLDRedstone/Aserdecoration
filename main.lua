...
DecoDialogShowing = function(plugin)
	...
	enumLocalization = {
		...
		--- key of enumLocalization[type] is the enum type name
		---@param type string
		---@param value string
		---@return string?
		...
		config.getPackedExpressions = function()
			...
			for i, expression in ipairs(config.expressions) do
				...
				if index then
					...
					sum = sum + 1
					index = sum
					local loopTypeStr = RDLoopTypeTranslation[expression.loop] or "onBeat"
					if config.loopOption ~= LoopType.Custom then
						loopTypeStr = RDLoopTypeTranslation[config.loopOption] or "onBeat"
					end
	...
	local function compressSprite(sprite, config)
		...
		local function getCompressedFrame(frame)
			...
			if #cmpsedFrames == 0 then
				cmpsedFrames[1] = { { frame = frame, duration = duration } }
				return { frame = 1, duration = duration }
			end
			for cflx, cf in pairs(cmpsedFrames) do
			...
			cmpsedFrames[#cmpsedFrames + 1] = { { frame = frame, duration = duration } }
			return { frame = #cmpsedFrames, duration = duration }
		end
		for i, expression in ipairs(config.expressions) do
	...
	local function compressImage(sprite, cmpsedFrames, config, filename)
		...
		if packMode == PackMode.HorizontalLine then
			size.width = #cmpsedFrames * sprite.width
			size.height = sprite.height
			sizeCount.width = #cmpsedFrames
			sizeCount.height = 1
		elseif packMode == PackMode.VerticalLine then
			size.width = sprite.width
			size.height = #cmpsedFrames * sprite.height
			sizeCount.width = 1
			sizeCount.height = #cmpsedFrames
		elseif packMode == PackMode.Pack then
			while sizeCount.width * sizeCount.height < #cmpsedFrames do
		...
		local framesUsed = {}
		for key, value in pairs(cmpsedFrames) do
